 ...
 * @param  {String} tokenName The name of the app to be used for the access token.
 ...
 */
module.exports = function (tokenName, tokenFile, scope) {
  ...
  //  can be created.
  return getUserCredentials(tokenName)
    .then((user) => {
      return authorizeApp(user, tokenName, scope);
    })
    .then((token) => {
      ...
      if (errors && appAlreadyExists(errors)) {
        print(clc.red.bold(`The app "${tokenName}" is already registered.`), true);
        print(['Visit ', clc.blue.underline('https://github.com/settings/tokens'), ` to delete "${tokenName}" from your `, clc.bold('Personal access tokens'), ', then run this script again.'], true);
      }
      process.exit(0);
    });
};
